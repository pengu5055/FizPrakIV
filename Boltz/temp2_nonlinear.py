...
fig, ax = plt.subplots()
fitpar, fitcov = curve_fit(LinearFit, xdata=T, ydata=Ic)
...
plt.errorbar(T, Ic, xerr=dT, markersize=4, color="#e326ed", linestyle='None',
             marker="o", capsize=2, label="Data", alpha=0.7)
plt.plot(T, Ic, c="#24C7F0", alpha=0.3)
plt.title(r"Odvisnost $I_C$ od $T$ pri $U_{BE}=0.57V$")
plt.xlabel(r'T [K]')
