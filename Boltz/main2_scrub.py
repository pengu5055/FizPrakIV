...
U = np.genfromtxt("Main2_Export.txt", usecols=0, skip_header=1)
Ic = np.genfromtxt("Main2_Export.txt", usecols=1, skip_header=1)

###### Data Scrub 2 outliers #######
U = np.array([U[i] for i in range(len(U)) if i != 0 and i != 1])
Ic = np.array([Ic[i] for i in range(len(Ic)) if i != 0 and i != 1])
...
plt.title(r"Linearizirana odvisnost $I_C$ od $U_{BE}$ pri $T=316K$")
